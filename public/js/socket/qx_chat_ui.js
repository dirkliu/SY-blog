...
function addMessageTo(_name, _time, _content) {
    var msg_list = $(".to-msg-list-body");
    _content = QxEmotion.Parse(_content);
    msg_list.append(
        '<div class="clearfix msg-wrap"><div class="msg-head">' +
        '<span class="msg-name label label-primary pull-left">' +
...
function addUserToList(_user) {
    $(".list-table").append('<tr><td>' + _user + '<span data-code=' + _user + ' style="float: right; color: red; cursor: pointer;">私聊他</span></td></tr>');
}
...
function onClickSendMessage() {
    ...
    // 如果第一个是@字符, 表示对某个人说（泛指私聊）
    if ( content.slice( 0,1 ) === "@" ) {
        content = content.slice( 1 ).split( " " );
        peopleToSay( content[0], content[1] );
    } else {
        say(content);
    }
...
$(".list-table").on( "click", "span", function() {
