...
socket.on( "people_to_say",function ( _nick_name, _content) {
    addMessageTo(_nick_name, getLocalHMS(), _content);
});
...
socket.on('user_change_nickname', function (_old_nick_name, _new_nick_name) {
    ...
    $('.list-table td span').each(function( index, value) {
        if ( value.dataset.code === _old_nick_name ) {
...
socket.on('user_join', function (_nick_name) {
    var num = 0;
    $('.list-table td span').each(function( index, value) {
        if ( value.dataset.code === _nick_name ) {
            num++;
        }
    });
...
// 对某一个人
function peopleToSay( name, _content) {
    socket.emit('people_to', name, _content);
}
